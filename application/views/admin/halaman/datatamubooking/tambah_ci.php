            ...
            $lama = ((abs(strtotime($keluar)-strtotime($masuk)))/(60*60*24));?>
                <div id="modalbct<?php echo $data['kode_booking']?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
                    ...
                    <div class="modal-header">
                        ...
                        <h3 class="modal-title" id="myModalLabel">Update Jadwal Booking</h3>
                    </div>
                    <form class="form-horizontal" role="form" method="post" action="<?php echo base_url('admin/dtbc/updatejadwalci') ?>" enctype="multipart/form-data">
                        ...
                        <div class="col-xs-9">
                            <div class="input-group date" id="tgl1<?php echo $data['kode_booking'] ?>">
                                <input type="text" class="form-control" name="tgl_masuk" value="<?php echo $masuk ?>" />   
                    ...
                    <label class="control-label col-xs-3" >Tanggal Check Out</label>
                        <div class="col-xs-9">
                            <div class="input-group date" id="tgl2<?php echo $data['kode_booking'] ?>">
...
$(function() { 
  ...
  $('#tgl1<?php echo $data['kode_booking'] ?>').datetimepicker({
   useCurrent: false,
   locale:'id',
   format:'YYYY-MM-DD'
   });
   
  $('#tgl2<?php echo $data['kode_booking'] ?>').datetimepicker({
   useCurrent: false,
   locale:'id',
   format:'YYYY-MM-DD'
   });
